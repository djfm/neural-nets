...
function Neuron (options) {
    ...
    this.compute = function compute (pass) {

        // Avoid running the same computation twice in a pass
        if (pass === this.pass) {
            return this.lastComputedValue;
        } else {
            this.pass = pass = this.pass + 1;
        }

        return this._compute_noCache(pass);
    };

    this._compute_noCache = function _compute_noCache (pass) {
        if (this.isInputNeuron()) {
            this.lastComputedValue = this.value;
        } else {
            this.lastComputedInput = _.reduce(this.inputs, function (sum, input) {
                return sum + input.connection.weight * input.neuron.compute(pass);
            }, 0);
